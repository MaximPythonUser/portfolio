...
const nextConfig = {
  ...
  // NOTE: убран turbopack.root="/" от GigaStudio (Linux-only, ломал сборку на Windows).
  // Сборка запускается через webpack: npm run build -- --webpack
  // (Turbopack падает на кириллице в пути "сайт портфолио").
  webpack: (config) => {
